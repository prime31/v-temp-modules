...
pub fn (state mut AppState) draw() {
	...
	state.batch.draw_text('right aligned', {x:512 + 300, y:384 - 300, rot:0, sx:state.scale, sy:state.scale align:.right_middle color:math.cornflower_blue() fontbook:state.font})
	...
	state.batch.draw_text('centered text', {x:512 + 300, y:384, rot: 0, sx: state.scale, sy: state.scale align:.center_middle color:math.orange()})
	state.font.set_blur(2)
	state.batch.draw_text('centered BLURRY text', {x:512 - 100, y:384, rot: state.rot, sx:4 sy:4 align:.center_middle color:math.orange()})
	state.batch.end()
